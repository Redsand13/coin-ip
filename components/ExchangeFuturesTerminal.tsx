...
export default function ExchangeFuturesTerminal({
    ...
    const fetchAndMerge = useCallback(async (tfOverride?: string, manual = false) => {
        ...
        try {
            ...
            setEntries(prev => {
                const merged = [...fresh, ...prev.filter(e => e.timeframe !== activeTf)]
    ...
    // Stable interval — background polls, no refreshing spinner
    useEffect(() => {
        fetchAndMerge();
        const id = setInterval(() => fetchAndMerge(), scanInterval);
        return () => clearInterval(id);
    }, [fetchAndMerge, scanInterval]);

    // Immediate fetch when TF changes — show spinner since user expects it
    const prevTf = useRef(timeframe);
    useEffect(() => {
        if (prevTf.current === timeframe) return;
        prevTf.current = timeframe;
        fetchAndMerge(timeframe, true);
    }, [timeframe, fetchAndMerge]);

    const deferredSearch = useDeferredValue(search);
    const filtered = useMemo(() => {
        const byTf = entries.filter(e => e.timeframe === timeframe);
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] text-muted-foreground hidden sm:inline">
                            {refreshing ? "Updating…" : `Live · ${scanInterval >= 60_000 ? `${scanInterval / 60_000}m` : `${scanInterval / 1_000}s`}`}
                        </span>
                    </div>
